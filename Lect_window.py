...
def teacher_window(ID):
    ...
    TP = Tk()
    TP.title("Educator Window")
    TP.geometry("800x700")
    TP.config(bg="#1b1b1b")

    #   pdfViewer
    display_schedule = pdf.ShowPdf().pdf_view(TP,
    ...
    # schedule_image = ImageTk.PhotoImage(Image.open("Images/calendar_1.png"))
    ...
    #   LabelFrame
    user_FN = LabelFrame(TP, text="User", bg="#1b1b1b", fg="#F0E6FA")
    course_FN = LabelFrame(TP, text="Modules", bg="#1b1b1b", fg="#F0E6FA")
    # schedule_FN = LabelFrame(TP, text="Schedule")
    logout_FN = LabelFrame(TP, text="Logout", bg="#1b1b1b", fg="#F0E6FA")
